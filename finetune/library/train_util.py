            ...
            for image_key, img_md in metadata.items():
                ...
                if os.path.exists(image_key):
                    ...
                    if os.path.exists(npz_path):
                        ...
                        assert len(abs_path) >= 1, f"no image : {image_key}"
                        abs_path = abs_path[0]
...
def load_tokenizer(args: argparse.Namespace):
    ...
    if tokenizer is None:
        # 检查本地路径是否存在，如果不存在，使用原始路径
        local_tokenizer_path = original_path
        if not os.path.exists(local_tokenizer_path):
            print(f"Local tokenizer path does not exist: {local_tokenizer_path}")
            raise FileNotFoundError(f"The specified local tokenizer path does not exist: {local_tokenizer_path}")

        if args.v2:
            # 确保子文件夹名称正确，例如 "tokenizer"
            tokenizer = CLIPTokenizer.from_pretrained(local_tokenizer_path, subfolder="tokenizer")
        else:
            tokenizer = CLIPTokenizer.from_pretrained(local_tokenizer_path)

    if hasattr(args, "max_token_length") and args.max_token_length is not None:
        print(f"update token length: {args.max_token_length}")
        tokenizer.model_max_length = args.max_token_length  # 更新最大长度
