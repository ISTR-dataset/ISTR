...
class ConfigSanitizer:
  ...
  def sanitize_argparse_namespace(self, argparse_namespace: argparse.Namespace) -> argparse.Namespace:
    try:
      ...
      print("Invalid cmdline parsed arguments. This should be a bug. ")
      raise
...
def generate_dreambooth_subsets_config_by_subdirs(train_data_dir: Optional[str] = None, reg_data_dir: Optional[str] = None):
  def extract_dreambooth_params(name: str) -> Tuple[int, str]:
    ...
    try:
      ...
      print(f"ignore directory without repeats : {name}")
      return 0, ""
...
def load_user_config(file: str) -> dict:
  ...
  if file.name.lower().endswith('.json'):
    try:
      ...
      print(f"Error on parsing JSON config file. Please check the format. : {file}")
      raise
  elif file.name.lower().endswith('.toml'):
    try:
      config = toml.load(file)
    except Exception:
      print(f"Error on parsing TOML config file. Please check the format.: {file}")
      raise
  else:
    raise ValueError(f"not supported config file format : {file}")
