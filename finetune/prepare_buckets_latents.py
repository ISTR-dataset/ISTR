...
def get_latents(vae, images, weight_dtype):
  ...
  print("Input image shape:", img_tensors.shape)
  with torch.no_grad():
...
def main(args):
  ...
  if not args.bucket_no_upscale:
    ...
    print("min_bucket_reso and max_bucket_reso are ignored if bucket_no_upscale is set, because bucket reso is defined by image size automatically ")
  ...
  def process_batch(is_last):
    for bucket in bucket_manager.buckets:
      if (is_last and len(bucket) > 0) or len(bucket) >= args.batch_size:
        ...
        if args.flip_aug:
          latents = get_latents(vae, [img[:, ::-1].copy() for _, img in bucket], weight_dtype)
          ...
          for image_key, _ in bucket:
            ...
            if os.path.isfile(npz_file_name):
              print(f"remove existing flipped npz: {npz_file_name}")
  ...
  for data_entry in tqdm(data, smoothing=0.0):
    ...
    if args.skip_existing:
      ...
      for npz_file in npz_files:
        ...
        if dat.shape[1] != reso[1] // 8 or dat.shape[2] != reso[0] // 8:
          found = False
          break
      if found:
        continue
...
def setup_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--in_json", type=str, default="", help="Input metadata file")
  parser.add_argument("--out_json", type=str, default="", help="Output metadata file")
  parser.add_argument("--model_name_or_path", type=str, default="",
                      help="Model name or path for retrieving latent features")
  parser.add_argument("--v2", action='store_true', help='Not used (retained for backward compatibility)')
  parser.add_argument("--batch_size", type=int, default=8, help="Batch size for inference")
  parser.add_argument("--max_data_loader_n_workers", type=int, default=2,
                      help="Number of worker threads for loading images with DataLoader (faster)")
  parser.add_argument("--max_resolution", type=str, default="256,256",
                      help="Maximum resolution during fine-tuning (width, height)")
  parser.add_argument("--min_bucket_reso", type=int, default=128, help="Minimum resolution for buckets")
  parser.add_argument("--max_bucket_reso", type=int, default=256, help="Maximum resolution for buckets")
  parser.add_argument("--bucket_reso_steps", type=int, default=32,
                      help="Step size for bucket resolution, preferably a multiple of 8")
  parser.add_argument("--bucket_no_upscale", action="store_true", help="Do not upscale images when creating buckets")
  parser.add_argument("--mixed_precision", type=str, default="no", choices=["no", "fp16", "bf16"],
                      help="Use mixed precision")
  parser.add_argument("--full_path", action="store_true",
                      help="Use full paths as image keys in metadata (supports multiple directories)")
  parser.add_argument("--flip_aug", action="store_true",
                      help="Apply flip augmentation to images and save the latent features of flipped images")
  parser.add_argument("--skip_existing", action="store_true",
                      help="Skip images if the npz file already exists (if flip_aug is enabled, check both normal and flipped images)")
  parser.add_argument("--recursive", action="store_true",
                      help="Recursively search for training labels in all subfolders of train_data_dir")
