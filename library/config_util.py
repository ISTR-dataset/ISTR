...
def add_config_arguments(parser: argparse.ArgumentParser):
  # 添加 dataset_config 参数，允许用户指定详细配置文件路径。
  parser.add_argument(
    "--dataset_config",
    type=Path,
    default=None,
    help="配置文件路径，用于详细设置 / config file for detail settings"
  )
...
class BaseSubsetParams:
  ...
  # edge_dir: Optional[str] = None  # 新增字段
  # image_dir_b: Optional[str] = None
  num_repeats: int = 1
...
class BlueprintGenerator:
  ...
  def generate(self, user_config: dict, argparse_namespace: argparse.Namespace, **runtime_params) -> Blueprint:
    ...
    for dataset_config in sanitized_user_config.get("datasets", []):
      ...
      for subset_config in subsets:
        params = self.generate_params_by_fallbacks(subset_params_klass,
                                                   [subset_config, dataset_config, general_config, argparse_config,
                                                    runtime_params])
        subset_blueprints.append(SubsetBlueprint(params))
...
def generate_dataset_group_by_blueprint(dataset_group_blueprint: DatasetGroupBlueprint):
  ...
  for dataset_blueprint in dataset_group_blueprint.datasets:
    # 根据 is_dreambooth 标志选择相应的子集和数据集类
    ...
    # 使用子集蓝图的参数生成子集实例，并将它们存入列表
    subsets = [subset_klass(**asdict(subset_blueprint.params)) for subset_blueprint in dataset_blueprint.subsets]


    # 使用生成的子集创建一个数据集实例，并添加到数据集列表中
  ...
  for i, dataset in enumerate(datasets):
    ...
    for j, subset in enumerate(dataset.subsets):
      ...
      # 根据数据集类型添加特定的 DreamBooth 或 FineTuning 配置
      if is_dreambooth:
        info += indent(dedent(f"""\n\
  ...
  # 创建数据集的分桶，因为分桶会影响数据集的长度，并且为所有数据集设置相同的随机种子
  seed = random.randint(0, 2 ** 31)  # 实际使用的种子为 seed + epoch_no
  for i, dataset in enumerate(datasets):
    print(f"[Dataset {i}]")
    dataset.make_buckets()  # 生成分桶
    dataset.set_seed(seed)  # 为数据集设置种子

  # 返回包含所有数据集的 DatasetGroup 实例
